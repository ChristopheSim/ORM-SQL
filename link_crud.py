...
def search_link(pk_link=False, fk_movie=False, fk_person=False, fk_role=False):
    try:
        engine = connect()
        ...
        if fk_movie and fk_person and fk_role:
            ...
            result = session.query(Link).filter(Link.fk_movie == fk_movie).all()
        elif fk_person:
            result = session.query(Link).filter(Link.fk_person == fk_person).all()
        elif fk_role:
            result = session.query(Link).filter(Link.fk_role == fk_role).all()
        elif pk_link:
